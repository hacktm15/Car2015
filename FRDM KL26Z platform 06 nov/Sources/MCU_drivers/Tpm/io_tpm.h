...
#define IO_TPM_PWM_CHANNEL_INDEX0 	0
#define IO_TPM_PWM_CHANNEL_INDEX1 	1
#define IO_TPM_PWM_CHANNEL_INDEX2 	2
#define IO_TPM_PWM_CHANNEL_INDEX3 	3
#define IO_TPM_PWM_CHANNEL_INDEX4 	4
#define IO_TPM_PWM_CHANNEL_INDEX5 	5
#define IO_TPM_PWM_CHANNEL_INDEX6 	6
#define IO_TPM_PWM_CHANNEL_INDEX7 	7
#define IO_TPM_PWM_CHANNEL_INDEX8 	8
#define IO_TPM_PWM_CHANNEL_INDEX9 	9
#define IO_TPM_PWM_CHANNEL_INDEX10	10
#define IO_TPM_PWM_CHANNEL_INDEX_NOT_CONF	0xFF
...
#define IO_TPM_CH_DMA_OFF 			0
#define IO_TPM_CH_DMA_ON			1

#define IO_TPM_COUNTER_START_VALUE 	0

/* Leonte start defines */

#define ERR_OK															0

#define IO_TPM_DMA_REQUEST_ON											1
#define IO_TPM_DMA_REQUEST_OFF											0
#define IO_TPM_CAPTURE_EDGE_NONE										0
#define IO_TPM_CAPTURE_RISING											1
#define IO_TPM_CAPTURE_FALLING											2
#define IO_TPM_CAPTURE_RISING_OR_FALLING 								3
...
typedef struct Io_Tpm_ChannelStatus_tag Io_Tpm_ChannelStatusStruct;
...
/* DESCRIPTION: This structure contains all data required for DCM - processing duty cycle / period */
struct Io_Tpm_ChannelStatus_tag{
	uint8 active_level : 1;
	uint8 state:2;
	uint8 ch_number;
	uint16 port_pin;
 ...
 * DESCRIPTION: Enable interrupt request for the selected TPM channel (input capture).
 * OBSERVATIONS: -
 */
extern uint8 Io_Tpm_DcmEnableInputCapture(uint32 channel);

/* FUNCTION NAME: Io_Tpm_DcmDisableInputCapture
 * RETURN TYPE: uint8  		ERR_OK = 0/ERR_X !=0;
 * PARAMETERS: uint32 channel
 * DESCRIPTION: Disable interrupt request for the selected TPM channel (input capture).
 * OBSERVATIONS: -
 */
extern uint8 Io_Tpm_DcmDisableInputCapture(uint32 channel);

 /* FUNCTION NAME: Io_Tpm_ProcessPeriod
  * RETURN TYPE: uint32
  * PARAMETERS: TPM_Type *tpm, uint32 tiks
  * DESCRIPTION: Evaluates the period depending on how many ticks has occurred and the tpm prescaler
  * OBSERVATIONS: + TODO:input clock freq
  */

/* FUNCTION NAME: Io_Tpm_GetPeriod
 * RETURN TYPE: void
 * PARAMETERS: uint32 channel , uint32 *address
 * DESCRIPTION: Stores in the specified address the channel's period
 * OBSERVATIONS: -
 */
extern void Io_Tpm_GetPeriod(uint32 channel , uint32 *address);

/* FUNCTION NAME: Io_Tpm_GetDutyCycle
 * RETURN TYPE: void
 * PARAMETERS: uint32 channel , uint32 *address
 * DESCRIPTION: Stores in the specified address the channel's dutycycle
 * OBSERVATIONS: -
 */
extern void Io_Tpm_GetDutyCycle(uint32 channel, uint32 *address);


extern uint32 Io_Tpm_GetPulseWidth(uint32 channel);
