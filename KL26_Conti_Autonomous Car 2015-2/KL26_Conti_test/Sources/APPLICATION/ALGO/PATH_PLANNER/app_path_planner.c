...
{
	if(Sens_sensorFrontDistanceCm < ALGO_ROBO_PP_FRONT_REAR_DIST_THOLD_CM)
	{
...
{
	if(Sens_sensorRearDistanceCm < ALGO_ROBO_PP_FRONT_REAR_DIST_THOLD_CM)
	{
		// Obstacle in front of car; car is put in PENDING state.
		Algo_Robo_Pp_CarState = ALGO_ROBO_PP_CAR_STATE_PENDING;
		Algo_Robo_Pp_TimerPendingState = 0;
...
void Algo_Robo_Pp_DetermineNextCarState(void)
{
	...
	case ALGO_ROBO_PP_CAR_STATE_SPOT_DETECTION:
	{
		...
		else
		{
			...
			if(Sens_sensorRightDistanceCm > ALGO_ROBO_PP_CAR_WIDTH_CM)
			{
			...
			else
			{
				...
				Algo_Robo_Pp_DistanceLastObstacleRightCm = Sens_sensorRightDistanceCm;
			}
		}
